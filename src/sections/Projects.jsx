...
function ProjectCard({ title, desc, tags, link }) {
  return (
    ...
    >
      <div className="mb-8 h-28 w-full rounded-xl bg-gradient-to-br from-white/10 to-transparent" />
      <h3 className="mb-1 text-lg font-semibold">{title}</h3>
      <p className="mb-4 text-sm text-white/70">{desc}</p>
      <div className="flex flex-wrap gap-2">
        {tags.map((t, i) => (<span key={i} className="rounded-lg border border-white/10 px-2 py-1 text-xs text-white/70">{t}</span>))}
      </div>
